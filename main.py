"""
Aurevia Pin Maker v6
- Opción A: CSV con drive_urls (como antes)
- Opción B: Subir imágenes desde PC + CSV con textos
- 16 layouts Pinterest
"""
...
app = FastAPI(title="Aurevia Pin Maker v6")
...
def load_from_bytes(data: bytes) -> Image.Image:
...
@app.get("/health")
def health():
    return {"status":"ok","version":"6.0","layouts":16}
...
<title>Aurevia Pin Maker v6</title>
<style>
...
.note{background:#fff8f5;border-left:4px solid #E8501C;padding:14px 16px;border-radius:8px;margin-top:22px;font-size:12px;color:#555;line-height:1.8}
...
<div class="box">
<h1>🖼 Aurevia Pin Maker <span style="background:#E8501C;color:#fff;font-size:11px;padding:3px 10px;border-radius:10px;margin-left:6px">v6</span></h1>
<p>Elige cómo subir las imágenes base:</p>

<div class="tabs">
  <div class="tab active" onclick="showTab('url')">🔗 Desde URL / Drive</div>
  <div class="tab" onclick="showTab('upload')">💻 Desde mi PC</div>
</div>
...
<div id="tab-url" class="panel active">
  ...
  <div class="note">
    ...
    <b>drive_url:</b> URL pública de Drive o PiAPI
  </div>
</div>

<!-- TAB B: Upload desde PC -->
<div id="tab-upload" class="panel">
  <form action="/process-upload" method="post" enctype="multipart/form-data">
    <label>🖼 Imágenes base (JPG/PNG, puedes seleccionar varias)</label>
  ...
  <div class="note">
    <b>Columnas CSV:</b> <code>filename, content_type, hook, body_text, cta, template</code><br>
    <b>filename</b> debe coincidir exactamente con el nombre del archivo de imagen<br>
    <b>Ejemplo:</b> si subes <code>pin_001.jpg</code>, el CSV debe tener <code>pin_001.jpg</code> en filename
  </div>
</div>

<div class="note" style="margin-top:16px">
  <b>16 Layouts disponibles:</b>
  <div class="grid">
...
async def process_upload(
    ...
    """Procesa imágenes subidas desde PC + CSV con textos"""
    # Load all uploaded images into a dict by filename
    image_map = {}
        ...
        for i, row in enumerate(reader,1):
            ...
            try:
                if fname not in image_map:
                    raise ValueError(f"Imagen '{fname}' no encontrada en los archivos subidos")
